...
import { onSelected } from "../../../reducer/ProjectReducer";
import { useStore } from "../../../store/StoreHooks";
import { completePhotoPath } from "../../../utils/helpers";

const ProjectMember = () => {
  const projectContext = useContext(ProjectContext);
  const { memberList, memberSelectedList, dispatch } = projectContext;
  ...
  const renderMembers = useMemo(() => {
    return memberList.map((member) => (
      <li className="list-group-item d-flex justify-content-between align-items-center border-0 py-50 px-0 cursor-pointer" key={member._id}>
        ...
        <div className="d-flex align-items-center cursor-pointer">
          <UserPlus size={17} className="mx-0" onClick={() => dispatch(onSelected(member, memberSelectedList, storeState.memberOrgList))} />
        </div>
      </li>
    ));
  ...
  const renderMembersSelected = useMemo(() => {
    return memberSelectedList.map((member) => (
      <li className="list-group-item d-flex justify-content-between align-items-center border-0 py-50 px-0 cursor-pointer" key={member._id}>
        ...
        <div className="d-flex align-items-center cursor-pointer">
          <X size={17} className="mx-0" onClick={() => dispatch(onSelected(member, memberSelectedList, storeState.memberOrgList))} />
        </div>
      </li>
    ));
